...
const parallelTracks = (week: Week) => {
  ...
  return [
    ...
    ["토익스피킹", "9월 말 IH 목표 · 말하기 5회와 주 1회 모의시험"],
  ];
};
...
const optimizedDailySchedule = [
  {
    ...
    events: [["오전 60~90분", "전동킥보드 강의 2강과 핵심 내용 기록"], ["오후 40분", "토익스피킹 약점 파트 재녹음 2회"], ["이후", "헬스 1시간 30분과 충분한 휴식"], ["20:30–21:00", "주간 회고와 다음 주 Speaking 약점 1개 선정"]],
  },
  {
    day: "월요일", type: "강의·회로 기초", focus: "주간 학습 흐름을 안정적으로 시작",
    events: [["출퇴근 30분", "토익스피킹 답변 구조·필수 표현 소리 내어 연습"], ["19:00–20:00", "저녁과 휴식"], ["20:00–20:25", "이번 주 회로 개념과 계산 복습"], ["20:30–21:10", "전동킥보드 강의 1강"], ["21:10–21:20", "배운 것 기록"]],
  },
  {
    day: "화요일", type: "헬스·짧은 복습", focus: "운동일에는 영어와 핵심 복습만 유지",
    events: [["출퇴근 30분", "토익스피킹 Part 1·2 녹음과 발음 교정"], ["19:00–21:00", "헬스·샤워·저녁"], ["21:30–21:55", "전동킥보드 강의 핵심 내용 복습"], ["21:55–22:05", "모르는 점과 다음 행동 기록"]],
  },
  {
    day: "수요일", type: "강의·HW 실습", focus: "운동이 없는 날에 실습 결과를 확보",
    events: [["출퇴근 30분", "토익스피킹 Part 3·4 질문별 즉답 연습"], ["19:00–20:00", "저녁과 휴식"], ["20:00–20:40", "전동킥보드 강의와 핵심 노트"], ["20:50–21:40", "관련 계산·회로·Firmware 실습"], ["21:40–21:50", "직접 한 것과 결과 기록"]],
  },
  {
    day: "목요일", type: "헬스·짧은 복습", focus: "운동 후 부담 없는 학습만 수행",
    events: [["출퇴근 30분", "토익스피킹 Part 5·6 의견 답변 녹음"], ["19:00–21:00", "헬스·샤워·저녁"], ["21:30–21:55", "이번 주 회로·MCU 개념 복습"], ["21:55–22:05", "이해하지 못한 점 기록"]],
  },
  {
    day: "금요일", type: "강의·취업 Evidence", focus: "강의 진도와 취업 기록을 가볍게 마무리",
    events: [["출퇴근 30분", "토익스피킹 미니 모의 1세트 녹음"], ["19:00–20:00", "저녁과 휴식"], ["20:00–20:40", "전동킥보드 강의 1강"], ["20:50–21:10", "녹음 1개 복기·막힌 표현 수정"], ["21:10–21:20", "취업 경험 또는 학습 결과 짧게 기록"]],
  },
  {
    day: "토요일", type: "헬스·강의", focus: "운동과 강의 진도를 함께 확보",
    events: [["오전 60~90분", "전동킥보드 강의 2강과 핵심 내용 기록"], ["오후", "헬스 1시간 30분·샤워·휴식"], ["저녁 45분", "토익스피킹 실전 모의 1회·녹음 복기"], ["20분", "강의에서 배운 것과 다음 실습 행동 정리"]],
  },
] as const;
...
function DailyAgenda({
  ...
  const maintenanceByDay = [
    ...
    ["토익스피킹", "Part 1·2 답변을 녹음하고 1회 수정"],
    ["토익스피킹", "Part 3·4 질문 5개 즉답 연습"],
    ["토익스피킹", "Part 5·6 의견 답변 2개 녹음"],
...
function Dashboard({
  ...
  const englishTasks = allDailyTasks.filter(
    (task) => task.track === "영어" || task.track === "토익스피킹" || /영어|토익스피킹|TOEIC|OPIc/i.test(task.text || ""),
  );
...
function CareerStrategy() {
  const sections = [
    ...
    },
    {
      ...
      tag: "9월까지 최우선",
      items: [
        "현재 TOEIC Speaking 90 → 2026년 9월 30일까지 IH 목표",
        "월~금 통근 30분: 답변 구조·필수 표현·파트별 녹음",
        "토요일 실전 모의 1회, 일요일 약점 답변 재녹음",
        "녹음마다 막힌 이유·수정 표현·다시 말한 결과 기록",
        "IH 취득 후 영어는 주 2회 유지하고 HW 학습 비중 확대",
      ],
      proof: "실제 IH 성적, 주간 모의 결과, 파트별 녹음과 수정 표현 기록",
    },
    {
      title: "3. 인적성",
      tag: "유지",
  ...
  return (
    ...
    >
      <div className="priority-banner">
        <b>준비 우선순위</b>
        <span>① 9월 토익스피킹 IH</span>
        <i>→</i>
        <span>② 전동킥보드 강의·회로</span>
        <i>→</i>
        <span>③ 인턴 경험 기록</span>
        <i>→</i>
        <span>④ 인적성·자소서·면접</span>
      </div>
      <article className="panel speaking-sprint">
        <div className="strategy-head"><h2>토익스피킹 IH · 9월 집중 계획</h2><span>병렬 유지</span></div>
        <div className="speaking-weeks">
          {[
            ["8/25–8/31", "진단·답변 틀", "실전 1회로 약점 파악 · 자주 쓸 답변 구조와 연결어 정리"],
            ["9/1–9/7", "발음·기초 응답", "Part 1·2 녹음 · 억양, 끊어 읽기, 사진 묘사 기본 문장 고정"],
            ["9/8–9/14", "즉답 속도", "Part 3·4 제한시간 연습 · 질문별 첫 문장 바로 말하기"],
            ["9/15–9/21", "의견 확장", "Part 5·6 이유 2개와 예시 1개를 시간 안에 완성"],
            ["9/22–9/30", "실전·시험", "주 3회 모의 · 반복 실수만 교정 · 시험 응시 후 실제 성적 기록"],
          ].map(([date, title, detail]) => <div key={date}><small>{date}</small><b>{title}</b><span>{detail}</span></div>)}
        </div>
        <p>IH를 받았다고 알려주면 토익스피킹은 유지 학습으로 바꾸고, 확보된 시간을 회로·전동킥보드 강의와 프로젝트에 자동 재배치합니다.</p>
      </article>
      <div className="strategy-grid">
        {[...sections].sort((a, b) => Number(b.title.includes("토익스피킹")) - Number(a.title.includes("토익스피킹"))).map((s) => (
  ...
  return (
    ...
    >
      <article className="schedule-rule">
        <b>일정 운영 원칙</b>
        <span>화·목 운동 후 20~30분 복습</span>
        <span>월·수·금 강의·실습 집중</span>
        <span>토·일 헬스와 강의 2강</span>
      </article>
